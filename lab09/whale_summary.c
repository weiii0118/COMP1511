...
void whale_summary(int n_sightings, struct pod sightings[n_sightings]) {
    struct tally sort[MAX_WHALE_SPECIES];
    int x=0,i=0;
    
    for(n_sightings=0 ; n_sightings<=1000 ; n_sightings++){
        for(x=0 ; x<=n_sightings ; x++){
            if(strcmp(sort[x].species,sightings[n_sightings].species)==0){
                sort[x].pods++;
                sort[x].whales+=sightings[n_sightings].how_many;
                break;
            }
        
            if(x==n_sightings){
                strcpy(sort[i].species,sightings[n_sightings].species);
                sort[i].pods=1;
                sort[i].whales=sightings[n_sightings].how_many;
                i++;
            }    
    ...
    for(x=0 ;x<=i;x++){
        ...
        printf("%d %s pods containing %d whales.\n",sort[x].pods,sort[x].species,sort[x].whales);
