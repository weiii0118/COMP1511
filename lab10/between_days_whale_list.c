...
int weighted(int year ,int month ,int day);
...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    int a=weighted(start_day->year,start_day->month,start_day->day);
    int b=weighted(finish_day->year,finish_day->month,finish_day->day);
    while(first_pod!=0){
        int c=weighted(first_pod->when->year,first_pod->when->month,first_pod->when->day);
        if(c>=a && c<=b){ 
