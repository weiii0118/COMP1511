...
int count_arkles(int tiles1[N_TILES], int tiles2[N_TILES]) {
    int n_arkles=0;
    int a=0,b=0,c=0,d=0;
    int tag1[4]={0},tag2[4]={0};
    for (a=0 ; a<N_TILES ; a++)
    {
        if(tiles1[a] == tiles2[a])
        {
            tag1[a]=1;
            tag2[a]=1;    
        }
...
void play_farnarkle(int hidden_tiles[N_TILES]) {
    ...
    while(farnarkles!=4)
    {
        printf("enter guess for turn %d: ",a);
        read_tiles(tiles);
        int farnarkles = count_farnarkles(hidden_tiles,tiles);
        printf("%d farnarkles,",farnarkles);   
        int arkles = count_arkles(hidden_tiles,tiles);
        printf("%d arkles\n",arkles);
        a=a+1;
        if(farnarkles==4)
        {
